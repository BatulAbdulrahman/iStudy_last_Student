...
class _TimeTableContainerState extends State<TimeTableContainer>
  ...
  if (context.read<AuthCubit>().isParent()) {
    context.read<AuthCubit>().authRepository.getProfile().then((profile) {
      final children = profile['data']?['children'] ?? [];

      // Find the matching child
      final studentDetails = children.firstWhere(
      ...
      if (studentDetails != null) {
        setState(() {
          getStudentClassDetails =
              studentDetails['class_section']?['full_name'] ?? "";
        });
      } else {
        setState(() {
          getStudentClassDetails = "Class details not found";
        });
      }
    }).catchError((error) {
      print("Error fetching profile: $error");
      setState(() {
        getStudentClassDetails = "Error fetching class details";
      });
    });
  } else {
    ...
    getStudentClassDetails =
        studentDetails.classSection?.fullName ?? "Class details not found";
  }
